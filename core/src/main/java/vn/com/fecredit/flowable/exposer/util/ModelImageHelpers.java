...
public final class ModelImageHelpers {
    ...
    public static byte[] createDmnPlaceholder(Object dmnModel, int width, int height) throws IOException {
        ...
        try {
            ...
            y += 30;
            for (String ln : lines) { g.drawString(ln, 44, y); y += 22; }
    ...
    public static byte[] createDmnDecisionTableImage(Object dmnModel, int width, int height) throws IOException {
        try {
            java.lang.reflect.Method getDecisions = dmnModel.getClass().getMethod("getDecisions");
            @SuppressWarnings("unchecked") List<Object> decisions = (List<Object>) getDecisions.invoke(dmnModel);
            ...
            try {
                ...
                for (int r = 0; r < renderRows; r++) {
                    cx = tableX;
                    if ((r & 1) == 0) { g.setColor(new java.awt.Color(255,255,255)); }
                    else { g.setColor(new java.awt.Color(250,250,255)); }
    ...
    public static byte[] createDmnDecisionTableImageFromXml(byte[] xmlBytes, int width, int height) throws IOException {
        try {
            ...
            if (tables.getLength() == 0) { tables = doc.getElementsByTagName("decisionTable"); if (tables.getLength() == 0) return null; }
            ...
            for (int r = 0; r < ruleEls.getLength(); r++) {
                ...
                for (int i = 0; i < inputs.size(); i++) {
                    if (i < inEntries.getLength()) { String txt = inEntries.item(i).getTextContent(); row.add(txt == null || txt.isBlank() ? "-" : txt.trim()); }
                    else { row.add("-"); }
                }
                org.w3c.dom.NodeList outEntries = rule.getElementsByTagName("outputEntry");
                for (int i = 0; i < outputs.size(); i++) {
                    if (i < outEntries.getLength()) { String txt = outEntries.item(i).getTextContent(); row.add(txt == null || txt.isBlank() ? "-" : txt.trim()); }
                    else { row.add("-"); }
                }
                rows.add(row);
            }
            ...
            String[] headers = new String[renderCols];
            for (int i = 0; i < renderCols; i++) { if (i < inCount) headers[i] = inputs.get(i); else headers[i] = outputs.get(i - inCount); }

            String[][] cells = new String[renderRows][renderCols];
            for (int r = 0; r < renderRows; r++) {
                java.util.List<String> src = rows.get(r);
                for (int c = 0; c < renderCols; c++) cells[r][c] = c < src.size() ? src.get(c) : "-";
            }
